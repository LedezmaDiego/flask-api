...
def albunes():
    ...
    anterior = None
    if pagina > 1:
       anterior = url_for('albunes', page=pagina-1, _external=True)
    if pagina < paginas:
       siguiente = url_for('albunes', page=pagina+1, _external=True)
    info = { 'count' : cant, 'pages': paginas,
             'next' : siguiente, 'prev' : anterior }
    res = { 'info' : info, 'results' : lista}
    return jsonify(res)

@app.route("/api/albun/<int:id>")
def albun(id):
   db = abrirConexion()
   ...
   cerrarConexion()
   if fila == None:
      return f"Álbum inexistente (id: {id})", 404
